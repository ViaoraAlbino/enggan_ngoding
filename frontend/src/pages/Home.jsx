    ...
    const Home = () => {
        ...
        // Bergeser otomatis setiap 3 detik
        useEffect(() => {
            autoSlideRef.current = setInterval(() => {
                nextSlide();
            }, 3000); // Ganti setiap 3 detik

            return () => clearInterval(autoSlideRef.current); // Bersihkan interval saat komponen dibersihkan
        }, [isAnimating]);

        return (
            <div className="relative w-full lg:h-[91vh] mt-16 mx-auto overflow-hidden">
                ...
                >
                    {slides.map((image, index) => (
                        <img
                            ...
                            src={image}
                            alt={`Slide ${index + 1}`}
                            className="w-full lg:h-[90vh] flex-shrink-0"
                        />
                    ))}
                </div>
